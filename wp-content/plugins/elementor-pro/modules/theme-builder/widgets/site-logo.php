...
class Site_Logo extends Widget_Image {
    ...
    // Get the control
    $caption_control = $this->get_controls('caption_source');

    // Check if the control exists and has an 'options' key
    if (isset($caption_control['options'])) {
        $caption_source_options = $caption_control['options'];

        // Remove the 'custom' option if it exists
        if (isset($caption_source_options['custom'])) {
    ...
    // Return an empty array or handle the error as needed if 'options' key doesn't exist
    return [];
}
	...
	return isset($site_logo['url']) ? $site_logo['url'] : Utils::get_placeholder_image_src();
}
}
